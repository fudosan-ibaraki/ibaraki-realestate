"""
茨城県 不動産価格分析 - グラフ03
駅距離別 平均価格・件数（マンション・宅地）タブ切り替え

【実行】
python graph_03_station_distance.py
"""
...
OUTPUT  = Path("graph_03_station_distance.html")

DIST_ORDER  = ["500m以内","500m〜1km","1〜2km","2〜3km","3〜5km","5km超"]
DIST_COLORS = ["#1B5E20","#2E7D32","#388E3C","#66BB6A","#A5D6A7","#C8E6C9"]
    ...
    df = pd.read_sql(f"""
        SELECT
            CASE
                ...
                WHEN nearest_station_dist <= 5000 THEN '3〜5km'
                ELSE '5km超'
            END                                     AS 駅距離区分,
            COUNT(*)                                AS 件数,
        ...
        GROUP BY 駅距離区分
    """, conn)
    df["駅距離区分"] = pd.Categorical(df["駅距離区分"], categories=DIST_ORDER, ordered=True)
    return df.sort_values("駅距離区分").reset_index(drop=True)


def df_to_json(df: pd.DataFrame) -> str:
    return json.dumps({
        "labels": df["駅距離区分"].tolist(),
...
def main():
    ...
    colors_json = json.dumps(DIST_COLORS)

    html = f"""<!DOCTYPE html>
<html lang="ja">
<head>
    <meta charset="UTF-8">
    <title>茨城県 駅距離別価格</title>
...
function updateGraph() {{
    ...
    const title   = currentTab === 'mansion'
        ? '茨城県 中古マンション 駅距離別 ' + (isPrice ? '平均価格' : '平均㎡単価')
        : '茨城県 宅地（土地と建物） 駅距離別 ' + (isPrice ? '平均価格' : '平均㎡単価');
    ...
    const traces = [{{
        ...
        hovertemplate: data.labels.map((l, i) =>
            `<b>${{l}}</b><br>` +
            (isPrice ? `平均価格: ${{data.price[i]}}万円` : `㎡単価: ${{data.sqm[i].toLocaleString()}}円`) +
            `<br>件数: ${{data.count[i].toLocaleString()}}件<extra></extra>`
        ),
        name: '',
    }}];
