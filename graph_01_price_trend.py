"""
茨城県 不動産価格分析 - グラフ01
全市区町村 平均価格推移（マンション・宅地）
チェックボックスで都市を絞り込み可能

【実行】
python graph_01_price_trend.py
"""
...
OUTPUT  = Path("graph_01_price_trend.html")
...
COLORS = [
    ...
    "#FFC107","#9C27B0","#03A9F4","#8D6E63","#546E7A",
    "#CDDC39","#FF4081","#00E5FF","#76FF03","#FFD740",
    "#FF6D00","#304FFE","#00BFA5","#AEEA00","#FFD600",
...
def main():
    ...
    print("データ取得中...")
    df_mansion = get_data(conn, "マンション")
    df_land    = get_data(conn, "宅地")
    conn.close()

    cities_m   = sorted(df_mansion["市区町村"].unique().tolist())
    cities_l   = sorted(df_land["市区町村"].unique().tolist())
    all_cities = sorted(set(cities_m + cities_l))

    print(f"マンション: {len(cities_m)}市区町村 / 宅地: {len(cities_l)}市区町村")

    data_m_json    = df_to_json(df_mansion)
    data_l_json    = df_to_json(df_land)
...
<div class="header">
    <h1>🏠 茨城県 不動産平均価格推移（2020〜2025年）</h1>
...
<div class="main">
    <div class="sidebar">
        ...
        <!-- マンション用チェックボックス -->
        <div id="cb-mansion" class="cb-list active">
            {cb_mansion_html}
        </div>
        <!-- 宅地用チェックボックス -->
        <div id="cb-land" class="cb-list">
            {cb_land_html}
        </div>
    </div>
...
function updateGraph() {{
    ...
    cities.forEach(city => {{
        ...
        traces.push({{
            ...
            hovertemplate: '<b>' + city + '</b><br>年: %{{x}}年<br>平均価格: %{{y}}万円<extra></extra>'
        }});
    }});

    Plotly.react('graph', traces, {{
        title: {{
            text: currentTab === 'mansion'
                ? '茨城県 中古マンション 市区町村別 平均価格推移'
                : '茨城県 宅地（土地と建物） 市区町村別 平均価格推移',
            font: {{ size: 16 }}, x: 0.5, xanchor: 'center'
        }},
        xaxis: {{
            title: '年', tickmode: 'linear',
            tick0: 2020, dtick: 1, gridcolor: '#EEEEEE'
        }},
        yaxis: {{
            title: '平均取引価格（万円）',
